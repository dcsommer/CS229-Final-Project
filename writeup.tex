...
\subtitle{Subtitle/extendend abstract}
%
...
Performing this optimization allowed us to run k-means clustering in XX\%
...
<Figure of dimension vs running time>

<Figure of dimension vs performance>  (where performance is measured as
the difference in how k-means labels the reduced data set set vs the
non-reduced)
...
\subsubsection{Fitting Sums of Gaussians}

Finally, we tried modelling the shape of each spike (per channel) as a
sum of Gaussian distributions. In this model, our feature vector is in
$\mathbb{R}^{2n}$ where $n$ is the number of Gaussian distributions we are
modelling with (each Gaussian has two real-valued parameters: $\mu, \
\sigma^2$).
